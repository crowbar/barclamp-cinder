...
node[:cinder][:volume].each do |backend_name, backend_params|
  ...
  when "emc"
    emc_config_filename ="/etc/cinder/cinder_emc_config-#{backend_name}.xml" 
    Chef::Log.info("Creating #{emc_config_filename} for emc backend #{backend_name}")
    template emc_config_filename do
      source "cinder_emc_config.xml.erb"
      owner node[:cinder][:user]
      group "root"
      mode 0640
      variables(:emc_params => backend_params[:emc])
    end
  when "rbd"
    #TODO/FIXME(saschpe): check/fix ceph::cinder cookbook!!!!
